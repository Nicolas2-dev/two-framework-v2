...
/**
 * ExceptionHandler convertit une exception en objet Response.
 *
 * Il est surtout utile en mode débogage pour remplacer le PHP/XDebug par défaut
 * sortie avec quelque chose de plus joli et de plus utile.
 *
 * Comme cette classe est principalement utilisée lors du démarrage du noyau, où rien n'est encore
 * disponible, le contenu de la réponse est toujours HTML.
 *
...
class DebugExceptionHandler
{
    ...
    /**
     * Enregistre le gestionnaire d'exceptions.
     *
     * @param bool $debug Activer/désactiver le mode débogage, où la trace de la pile est affichée
     * @param string|null $charset Le jeu de caractères utilisé par les messages d'exception
     * @param string|null $fileLinkFormat Le modèle de lien IDE
     *
     * @return static
     */
    public static function register($debug = true, $charset = null, $fileLinkFormat = null)
    {
        ...
        if (\is_array($prev) && $prev[0] instanceof DebugErrorHandler) {
     ...
     *
     * @return callable|null Le gestionnaire d'exceptions précédent, le cas échéant
     */
    ...
    /**
     * Définit le format des liens vers les fichiers sources.
     *
     * @param string|FileLinkFormatter $fileLinkFormat Le format des liens vers les fichiers sources
     *
     * @return string Le format de lien de fichier précédent
     */
    ...
    /**
     * Envoie une réponse pour l'exception donnée.
     *
     * Pour être aussi sûr que possible, l'exception est d'abord gérée
     * par notre simple gestionnaire d'exceptions, puis par le gestionnaire d'exceptions utilisateur.
     * Ce dernier est prioritaire et toute sortie du premier est annulée,
     * si et seulement si rien de grave ne se produit dans ce chemin de traitement.
     */
    public function handle(\Exception $exception)
    {
        ...
        while (null === $this->caughtBuffer && ob_end_flush()) {
            // Boucle vide, tout est dans l'état
        }
        if (isset($this->caughtBuffer[0])) {
            ob_start(function ($buffer) {
                if ($this->caughtLength) {
                    // utilisez substr_replace() au lieu de substr() pour la résistance à la surcharge mbstring
        ...
        try {
            ...
            if (!$caughtLength) {
                // Tous les gestionnaires ont échoué. Laissez PHP gérer cela maintenant.
                throw $exception;
            }
        }
    }
    ...
    /**
     * Envoie l'erreur associée à l'exception donnée sous forme de réponse PHP simple.
     *
     * Cette méthode utilise des fonctions PHP simples comme header() et echo pour afficher la sortie.
     * la réponse.
     *
     * @param \Exception|FlattenException $exception Une instance \Exception ou FlattenException
     */
    ...
    /**
     * Obtient le contenu HTML complet associé à l'exception donnée.
     *
     * @param \Exception|FlattenException $exception Une instance \Exception ou FlattenException
     *
     * @return string Le contenu HTML sous forme de chaîne
     */
    ...
    /**
     * Obtient le contenu HTML associé à l'exception donnée.
     *
     * @return string Le contenu sous forme de chaîne
     */
...
EOF
            ...
            // quelque chose de désagréable s'est produit et nous ne pouvons plus lancer d'exception
            if ($this->debug) {
...
EOF;
    ...
    /**
     * Obtient la feuille de style associée à l'exception donnée.
     *
     * @return string La feuille de style sous forme de chaîne
     */
...
EOF;
    ...
    /**
     * Formate un tableau sous forme de chaîne.
     *
     * @param array $args Le tableau d'arguments
     *
     * @return string
     */
